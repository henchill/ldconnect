...
exports.getProfileFromWebid = function(uri, callback) {
	...
	fetch.nowOrWhenFetched(docURI, undefined, function(ok, body, xhr) { // @@ check ok
		...
		// Figure out
		// var preferencesworkspaces = 
		if (name) person_info.name = name.value;
		if (preferencesfile) person_info.preferencesfile = preferencesfile.value;
...
exports.getConfig = function(args, callback) {
	...
	var comments = kb.statementsMatching(undefined, RDF('type'), SIOC('Post'));
	var configurl = args.configurl + "?key=" + constants.ACL_KEY;
	debug(args.configurl);
	fetch.nowOrWhenFetched(configurl, undefined, function(ok, body, xhr) { // @@ check ok
		if (!ok) {
			debug("Failed to fetch configfile");
			debug(body);
			callback({ error: "could not fetch config file"});
			return;
		}
		...
		if (configs.length > 0) {
			...
			for (var i = 0; i < args.fields; i++) {
				...
				if (args.fields[i] === "email") {
					var email = kb.any(configitem, LDFB("email"));
					if (email) {
				...
				if (args.fields[i] === "password") {
					var password = kb.any(configitem, LDFB("password"));
					if (password) {
